...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group17/Test_Case_02/case2a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group17/Test_Case_02/case2a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group17/Test_Case_02/case2a/inputrequest/request3.json	${asset_Id_Product1}
...
11. Get Reviewer Summary Questions Details
    [Tags]	Functional	GET	current
	set global variable  ${asset_assmnt_clause_id}    ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ00
	run keyword and ignore error  Get Reviewer Summary Questions Details    ${asset_Id_Product1}    ${asset_assmnt_clause_id}   status
#	${error_msg}  Get Error Message  ${reviewer_summary_api}
    ${error_msg}  Get Error Message  ${response_api}
    should be equal  "${error_msg}"  "No data found"