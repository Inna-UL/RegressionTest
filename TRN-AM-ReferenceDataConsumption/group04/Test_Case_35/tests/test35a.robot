...
11. Get certificate Details
    [Tags]	Functional	certificate    current
    run keyword and ignore error  Get certificate Details using Mode     ${EMPTY}    view
#    run keyword if  "${response}" != "('FAIL', u'HTTPError: HTTP Error 400: Bad Request')"     Fail
#    ${error_msg}  Get Error Message  ${response_api}
#    should be equal  "${error_msg}"  "Invalid Certificate Id or Private Label Id is passed in URL"
