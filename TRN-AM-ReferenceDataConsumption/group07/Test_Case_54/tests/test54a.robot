...
12. Certify certificate
    [Tags]	Functional	certificate	create	POST    current
    ${response}  run keyword and ignore error  Add Decisions to Certificate using skipValidation  Certificate/Certify_RegressionCertificate.json     ${EMPTY}    false
#    should be equal  ${status_404}   'HTTP Error 404: Not Found'
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'JSONObject[\"certificateName\"] not found.'  Fail	test1 Teardown
	log to console  ${msg}