...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group22/Test_Case_125/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group22/Test_Case_125/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group22/Test_Case_125/inputrequest/request3.json	${asset_Id_Product2}
...
14. Get Compoenent of Asset in Collection
    [Tags]	Functional	current
    ${response}  Get Component of Asset In Collection   ${Product_Collection_Id}
    run keyword if  '${response}' != '${asset_Id_Product2}'    Fail 	test1 Teardown
