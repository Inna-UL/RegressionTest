...
#    run keyword if  ${response._content} != {"apiversion":"${Api_ver}","code":200,"status":"OK","message":"OK","data":{"id":"${Collection_Id}","status":"Collection locked successfully"}}  Fail	test1 Teardown
    run keyword if  ${response._content} != {"code":200,"status":"OK","message":"OK","data":{"id":"${Collection_Id}","status":"Collection locked successfully"}}  Fail	test1 Teardown
...
#    run keyword if  ${response._content} != {"apiversion":"${Api_ver}","code":200,"status":"OK","message":"OK","data":{"id":"${asset_Id_Product1}","status":"Asset un-locked successfully"}}  Fail	test1 Teardown
    run keyword if  ${response._content} != {"code":200,"status":"OK","message":"OK","data":{"id":"${asset_Id_Product1}","status":"Asset un-locked successfully"}}  Fail	test1 Teardown
    log to console  ${response._content}

8. Unlock Collection
    [Tags]	Functional	current
    run keyword and ignore error  Unlock Collection    Unlock_Template_emptyUser.json    ${Collection_Id}
#    run keyword if  ${response_api} != {"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"Collection is already unlocked"}       Fail	test1 Teardown
    run keyword if  ${response_api} != {"code":400,"status":"Bad Request","message":"Collection is already unlocked"}       Fail	test1 Teardown
