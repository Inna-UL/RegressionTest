...
7. Unlock Collection
    [Tags]	Functional	current
    ${response}  Unlock Collection    Unlock_Template.json    ${Collection_Id}
#    run keyword if  ${response._content} != {"apiversion":"${Api_ver}","code":200,"status":"OK","message":"OK","data":{"id":"${Collection_Id}","status":"Collection un-locked successfully"}}  Fail	test1 Teardown
    run keyword if  ${response._content} != {"code":200,"status":"OK","message":"OK","data":{"id":"${Collection_Id}","status":"Collection un-locked successfully"}}  Fail	test1 Teardown
    log to console  ${response._content}
