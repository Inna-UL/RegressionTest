...
21. Search Certificate with Offset as '0' and Rows values as 'Empty'
	...
	Paginated Search for PL Certificate    PL_Certificate_Search_with_searchText_offset&rows.json
	Extract certificate search response   ${certificate_search}
	should not be empty  ${certificate_total_count}
	should be equal  ${certificate_offset}   ${value_as_0}
...
21a. Validate PL Certificate Details
    ...
    should be equal  ${is_privateLabel}  [${value_as_true}, ${value_as_true}]
	...
	compare lists  [${unique_certificateId}, ${CS_certificate_Id}, ${CS_certificate_hierarchyId}, ${CS_partySiteContainerId}]    [["${PrivateLabel_Id2}","${PrivateLabel_Id}"], ["${PrivateLabel_Id2}", "${PrivateLabel_Id}"], ["${certificate_hierarchy_Id}", "${certificate_hierarchy_Id}"], ["${EMPTY}", "${EMPTY}"]]
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_1}"], ["PL2-${certificate_name_1}-${current_time}", "PL-${certificate_name_1}-${current_time}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_1}"], ["${Scope_Code_1}", "${Scope_Code_1}"]]
