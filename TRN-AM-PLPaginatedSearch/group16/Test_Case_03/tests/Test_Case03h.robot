...
21. Search Certificate with Offset as '0' & Rows as '10'
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${offset_value}  ${value_as_0}
	set global variable  ${rows_value}  ${value_as_10}
	...
	should be equal  ${certificate_rows}   ${value_as_10}
...
21a. Validate PL Certificate Details
	...
	compare lists  [${unique_certificateId}, ${CS_certificate_Id}, ${CS_certificate_hierarchyId}, ${CS_partySiteContainerId}]    [["${PrivateLabel_Id}","${PrivateLabel_Id2}"], ["${PrivateLabel_Id}","${PrivateLabel_Id2}"], ["${certificate_hierarchy_Id}", "${certificate_hierarchy_Id}"], ["${EMPTY}", "${EMPTY}"]]
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_1}"], ["PL-${certificate_name_1}-${current_time}", "PL2-${certificate_name_1}-${current_time}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_1}"], ["${Scope_Code_1}", "${Scope_Code_1}"]]
