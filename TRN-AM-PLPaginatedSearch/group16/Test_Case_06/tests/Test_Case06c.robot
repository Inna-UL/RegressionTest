...
21. Search Certificate with "isPLCertificate" field:'Empty'
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${searchText_value}    ${certificate_name_1}-${current_time}
	log to console      ${certificate_name_1}-${current_time}
	set global variable  ${isPLCertificate_value}    ${EMPTY}
	Paginated Search for PL Certificate    PL_Certificate_Search_with_searchText.json
...
21a. Validate PL Certificate Details
	...
	compare lists  [${unique_certificateId}, ${CS_certificate_Id}, ${CS_certificate_hierarchyId}, ${CS_partySiteContainerId}]    [["${PrivateLabel_Id}", "${Certificate_Id}"], ["${PrivateLabel_Id}", "${Certificate_Id}"], ["${certificate_hierarchy_Id}", "${certificate_hierarchy_Id}"], ["${EMPTY}", "${EMPTY}"]]
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_1}"], ["PL-${certificate_name_1}-${current_time}", "${certificate_name_1}-${current_time}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_1}"], ["${Scope_Code_1}", "${Scope_Code_1}"]]
	compare lists  [${CS_issueDate}, ${CS_revisionDate}, ${CS_withdrawalDate}, ${CS_expiryDate}]     [["${today_date} ${time_00}", "${today_date} ${time_00}"], ["${EMPTY}","${EMPTY}"], ["${EMPTY}","${EMPTY}"], ["${EMPTY}", "${EMPTY}"]]
#    Extract values from parties list     ${parties}
#    compare lists    ${partySiteNumber_list}    ["${BO_partysite_number_1}", "${PS_partysite_number_1}", "${AP_partysite_number_1}", "${LR_partysite_number_1}"]
#    compare lists    ${accountNumber_list}   ["${BO_account_number_1}", "${PS_account_number_1}", "${AP_account_number_1}", "${LR_account_number_1}"]
#    compare lists    ${relationshipType_list}    ["${party_brand_owner}", "${party_production_site}", "${party_applicant}", "${party_local_representative}"]
