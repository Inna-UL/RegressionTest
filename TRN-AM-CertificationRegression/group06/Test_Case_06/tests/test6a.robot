...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group06/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group06/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group06/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
17. Certificate Audit with missing Certificate Params
    [Tags]	Functional	current
    run keyword and ignore error  Certificate Audit  certificateType=Regression%20Scheme&certificateName=${Certificate_Name}
	${error_msg}  Get Error Message  ${response_api}
    should be equal  "${error_msg}"  "'ownerReference' mandatory key missing in query parameter"
