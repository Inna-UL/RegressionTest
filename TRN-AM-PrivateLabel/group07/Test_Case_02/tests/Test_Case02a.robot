...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group07/Test_Case_02/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group07/Test_Case_02/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group07/Test_Case_02/inputrequest/request3.json	${asset_Id_Product1}
...
19. View Private Label Asset
    [Tags]	Functional	certificate	PL  create	POST    current
    set global variable  ${PrivateLabel_Asset_Id}    ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ00
    run keyword and ignore error  View Private Label Assets    ${PrivateLabel_Asset_Id}
