...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_04/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group08/Test_Case_04/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group08/Test_Case_04/inputrequest/request3.json	${asset_Id_Product1}
...
20. Lock Private Label Certificate
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  Lock Private Label   Lock_Template_empty.json  ${PrivateLabel_Id}
