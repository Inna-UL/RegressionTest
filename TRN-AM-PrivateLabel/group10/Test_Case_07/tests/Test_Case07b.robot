...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group10/Test_Case_07/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group10/Test_Case_07/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group10/Test_Case_07/inputrequest/request3.json	${asset_Id_Product1}
...
20. Certify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificate.json

21. View Private Label Parties Details
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  View Private Label Parties Details    ${PrivateLabel_Id}
