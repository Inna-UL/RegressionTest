...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group10/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group10/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group10/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
18. View Private Label Details
    [Tags]	Functional	certificate	PL  current
    set global variable  ${PrivateLabel_Id}  ABCDEFG-HIJK-LMNO-PQRSTUVWXYZ
    run keyword and ignore error  View Private Label Details   ${PrivateLabel_Id}
