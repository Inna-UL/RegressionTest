...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group02/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
19. View Private Label Asset
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  View Private Label Assets Details    ${empty}
#    should be equal  ${url_response}    HTTP Error 404: Not Found
#     should be equal  ${response_api} = {"apiversion":"${Api_ver}","code":500,"status":"Internal Server Error","message":"Query parameters missing"}
     should be equal  ${response_api} = {"code":500,"status":"Internal Server Error","message":"Query parameters missing"}