...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group17/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group17/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group17/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
18. Get Base Certificate Details for a Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error   Get Base Certificate Details for a Private Label   abcdef
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Invalid Private Label Id passed in URL'  Fail	test1 Teardown
	log to console  ${msg}
#    ${response}  get error message  ${response_search_api}
#    run keyword if	"${response}" != "Invalid Private Label Id passed in URL"	Fail	test1 Teardown
#    log to console  ${response}
