...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group05/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
18. Search for the Private Label Assets
    [Tags]	Functional	create  POST	current
    run keyword and ignore error  Search Private label Asset    productType=Regression%20Test%20Product%201
#    run keyword if  '${response_api}' != '${PrivateLabel_Id}'     Fail   test1 Teardown
