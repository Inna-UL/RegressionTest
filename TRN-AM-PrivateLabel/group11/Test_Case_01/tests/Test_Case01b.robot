...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group11/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group11/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group11/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
16. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group02/Test_Case_12/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_12/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_12/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
...
23. Add Asset To Private Label(PL1)
    [Tags]	Functional	certificate	PL  create	POST    current
...
24. Create Private Label(PL2)
    [Tags]	Functional	certificate	PL  create	POST    current
    Create private label    Private_Label/CreationOfPrivateLabel_RegressionSchemeCertificate_Cert2.json

25. Add Asset To Private Label(PL2)
    [Tags]	Functional	certificate	PL  create	POST    current
    Add Asset To PL  Private_Label/Add_Asset_To_Private_Label_With_Different_Taxonomy.json
    ${response}  Get Private Lable Asset ID   ${response_api}
    should not be empty  ${response}
    log to console   Private Lable Asset_ID2: ${response}
    set global variable  ${PL_Asset_Id2}    ${response}

26. View Private Label Assets of a base asset
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  View Private Label Assets of a base Asset_two_asset_Id   ${asset_Id_Product1}
    run keyword if  ${response} != ['${PL_Asset_Id1}', '${PL_Asset_Id2}']  Fail	test1 Teardown