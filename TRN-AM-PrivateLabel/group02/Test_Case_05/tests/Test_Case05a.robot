...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group02/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
18. Add Asset To Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    set global variable  ${asset_Id_Product1}    ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ00
    run keyword and ignore error  Add Asset To PL  Private_Label/Add_Asset_To_Private_Label.json
    ${response}  Get Private Label Error Message  ${response_api}
    run keyword if  ${response} != "Platform does not support linking this asset. Please see your SME for further instructions."   Fail  test1 Teardown
    log to console  ${response}
