...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group02/Test_Case_03/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_03/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_03/inputrequest/request3.json	${asset_Id_Product1}
...
17. Add Asset To Private Label
    ...
    ${response}  run keyword and ignore error    Add Asset To PL  Private_Label/Add_Asset_To_Private_Label.json
#    run keyword if	${response} != ('FAIL', u'200 != 404')  Fail	test1 Teardown
    run keyword if	${response} != ('FAIL', u'200 != 405')  Fail	test1 Teardown
    log to console  ${response}