...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group13/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group13/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group13/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
18. Add Party To Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    ${result}  Add Party To PL  Private_Label/Add_Parties_To_Private_Label.json
    ${party_id}  Local Reresentative Party Id  ${result}
    set global variable  ${PrivateLabel_Party_Id}   ${party_id}

19. Unlink Party From Private Label
    [Tags]	Functional	certificate	PL	POST    current
    set global variable  ${PrivateLabel_Party_Id}   ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ
    run keyword and ignore error  Unlink Party from Private Label  Unlink_Asset_Template.json  ${PrivateLabel_Party_Id}
#    run keyword if  '${response_api}' != '{"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"Invalid Platform Party Id passed in URL"}'  Fail	test1 Teardown
    run keyword if  '${response_api}' != '{"code":400,"status":"Bad Request","message":"Invalid Platform Party Id passed in URL"}'  Fail	test1 Teardown
