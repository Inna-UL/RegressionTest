...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group15/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group15/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group15/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
19. View Base Asset of a private label asset
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error  View Base Asset of a private label asset    abcdef
