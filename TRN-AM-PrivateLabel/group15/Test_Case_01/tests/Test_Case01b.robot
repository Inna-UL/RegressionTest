...
26. View Base Asset of a private label asset by providing plAssetId1
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  View Base Asset of a private label asset  ${PrivateLabel_Asset_Id}
    run keyword if  '${response}' != '${asset_Id_Product1}'     Fail   test1 Teardown
    log to console  Asset ID: ${response}