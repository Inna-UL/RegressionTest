...
10. Render Verdict
	[Tags]	Functional	POST	notcurrent
	${response1}	Render Verdict  group04/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
18. Search for the Private Label
    ...
    ${response}    Search private label    certificateType=Regression%20Scheme&mark=Listed&productType=Regression%20Test%20Product%201
#    run keyword if  '${response}' != '${PrivateLabel_Id}'     Fail
