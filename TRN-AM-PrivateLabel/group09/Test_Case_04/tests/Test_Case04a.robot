...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group09/Test_Case_04/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group09/Test_Case_04/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group09/Test_Case_04/inputrequest/request3.json	${asset_Id_Product1}
...
21. Unlock Private Label Certificate
    [Tags]	Functional	certificate	PL  POST    current
    set global variable  ${PrivateLabel_Id}  ${EMPTY}
    ${response}  run keyword and ignore error  Unlock Private Label   Unlock_Template.json  ${PrivateLabel_Id}
#    run keyword if  ${response} != '{"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"JSONObject[\"privateLabelIds\"] not found."}'   Fail	test1 Teardown
    run keyword if  ${response} != ('FAIL', u'200 != 400')  Fail   test1 Teardown
    log to console  ${response}
