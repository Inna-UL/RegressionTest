...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group09/Test_Case_03/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group09/Test_Case_03/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group09/Test_Case_03/inputrequest/request3.json	${asset_Id_Product1}
...
21. Unlock Private Label Certificate
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  run keyword and ignore error   Unlock Private Label   Unlock_Template_emptyUser.json  ${PrivateLabel_Id}
#    run keyword if  '${response_api}' != '{"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"Nisarg_Automation is currently editing this certificate"}'  Fail   test1 Teardown
    run keyword if  '${response_api}' != '{"code":400,"status":"Bad Request","message":"Nisarg_Automation is currently editing this certificate"}'  Fail   test1 Teardown
    log to console  ${response}