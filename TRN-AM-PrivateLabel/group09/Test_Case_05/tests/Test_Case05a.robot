...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group09/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group09/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group09/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
20. Lock Private Label Certificate
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  Lock Private Label   Lock_Template.json  ${PrivateLabel_Id}
#    run keyword if  '${response}' != '{"apiversion":"${API_VER}","code":200,"status":"OK","message":"OK","data":{"Message":"Privatelabel locked successfully"}}'  Fail   test1 Teardown
    run keyword if  '${response}' != '{"code":200,"status":"OK","message":"OK","data":{"Message":"Privatelabel locked successfully"}}'  Fail   test1 Teardown
    log to console  ${response}

21. Unlock Private Label Certificate
    [Tags]	Functional	certificate	PL  POST    current
    set global variable  ${PrivateLabel_Id}  ABCDEF-GHIJ-KLMNOPQR-STUVWXYZ
    ${response}  run keyword and ignore error  Unlock Private Label   Unlock_Template.json  ${PrivateLabel_Id}
#    run keyword if  '${response_api}' != '{"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"Invalid Private Label Id passed in URL"}'  Fail   test1 Teardown
    run keyword if  '${response_api}' != '{"code":400,"status":"Bad Request","message":"Invalid Private Label Id passed in URL"}'  Fail   test1 Teardown
    log to console  ${response_api}