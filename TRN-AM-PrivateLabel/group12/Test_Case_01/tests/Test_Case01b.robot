...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group12/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group12/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group12/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
18. Add Asset To Private Label
    ...
    ${result}   Add Asset To PL  Private_Label/Add_Asset_To_Private_Label.json
    set global variable  ${PrivateLabel_Asset_Id}   ${result}

19. Unlink Asset From Private Label
    [Tags]	Functional	certificate	PL	POST    current
    set global variable  ${PrivateLabel_Asset_Id}   ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ
    run keyword and ignore error  Unlink Asset from Private Label  Unlink_Asset_Template.json  ${PrivateLabel_Asset_Id}
#    run keyword if  '${response_api}' != '{"apiversion":"${Api_ver}","code":400,"status":"Bad Request","message":"PL Asset data does not exists"}'  Fail	test1 Teardown
    run keyword if  '${response_api}' != '{"code":400,"status":"Bad Request","message":"PL Asset data does not exists"}'  Fail	test1 Teardown
