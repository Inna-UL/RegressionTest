...
14. Edit Private Label Asset without Private Label ID
    [Tags]	Functional	certificate	PL  create	POST    current
    ${response}     run keyword and ignore error     Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_Model_Nomenclature.json   ${empty}
#    run keyword if  ${response} != ('FAIL', u'200 != 404')  Fail
    run keyword if  ${response} != ('FAIL', u'200 != 400')  Fail
