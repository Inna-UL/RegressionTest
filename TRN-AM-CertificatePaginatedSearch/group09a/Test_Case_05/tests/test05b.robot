...
19. Search Certificate with searchText and "latestCompletedCertificate" field as "N"
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${searchText_value}    ${certificate_name_1}-${current_time}
	set global variable  ${latestCompletedCertificate_value}    ${value_as_N}
...
19a. Validate Certificate Details
    [Tags]	Functional	asset   Search	POST    current
    ...
    Compare lists  [${certificate_status}, ${certificate_version}, ${revision_number}, ${certify}]   [["${status_Under_Revision}"], ["${value_as_3.0}"], ["${value_as_3}"], ["${value_as_N}"]]
...
19b. Validate findKeys searchText Details
    [Tags]	Functional	asset   Search	POST    current
