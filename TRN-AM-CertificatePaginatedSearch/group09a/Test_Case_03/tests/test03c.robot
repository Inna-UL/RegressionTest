...
8. Associate parties to certificate
    [Tags]	Functional	certificate	create	POST    current
    ${has_assets}    Get HasAssets using Certificate Details   ${certificate_type_1_url}   ${certificate_name_1}-${current_time}    ${Cert_Owner_Ref}   ${Certificate_Id}
    Should not be Empty     ${has_assets}
    ${has_evaluations}    Get HasEvaluations using Certificate Details   ${certificate_type_1_url}   ${certificate_name_1}-${current_time}    ${Cert_Owner_Ref}   ${Certificate_Id}
...
12. Search Certificate with Offset as Empty & Rows as '2'
